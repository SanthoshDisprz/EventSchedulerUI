...
const SearchButton = ({ handleSearchLocation, handleSearchResult, handleCurrentLocation, onSearch}) => {
  const [searchLocation, setSearchLocation] = useState("")
    return ( 
    <div className="search-input-container" onKeyUp={(e)=>{if(e.key==="Enter"){handleSearchLocation(searchLocation); onSearch(true)}}}>
      ...
      <SlMagnifier className="search-icon" onClick={()=>{handleSearchLocation(searchLocation); onSearch()}}/>
    </div>
 );
}
